...
export const DarkMarkBounty: React.FC = () => {
  ...
  const [timeLeft, setTimeLeft] = useState(() => {
    const savedEndTime = localStorage.getItem("dark_mark_end_time");
    if (savedEndTime) {
    ...
    localStorage.setItem("dark_mark_end_time", newEndTime.toString());
    return duration;
  });

  useEffect(() => {
    const timer = setInterval(() => {
      const endTimeStr = localStorage.getItem("dark_mark_end_time");
      if (endTimeStr) {
  ...
  const timerState = { timerStatus: timeLeft > 0 ? "running" : "ended" };
  ...
  return (
    <div className="dark-mark-bounty-root">
      <div style={{ height: '100%' }}>
        ...
        <div style={{ position: "absolute", top: 20, right: 20, color: timeLeft <= 300 ? "#ff4444" : "#FFD700", fontWeight: "bold", fontSize: "1.2rem", zIndex: 100, background: "rgba(0,0,0,0.5)", padding: "10px 15px", borderRadius: "10px", border: "1px solid rgba(212,175,55,0.3)" }}>
          {timerState.timerStatus === "ended" ? "TIME UP" : formattedTime}
        </div>
