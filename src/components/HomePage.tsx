        ...
        <div className="min-h-screen bg-black text-white pb-20">
            ...
            {showIntro ? (
                ...
                <div className="min-h-screen bg-black text-white pb-20 animate-in fade-in duration-1000">
                    ...
                    {/*  CONDITIONAL NAVBAR (Themed to match Technetics) */}
                    {showNavbar && (
                        <nav className="sticky top-0 z-50 bg-[#1a0f08]/95 backdrop-blur-md border-b-2 border-[#8b6e2e]/30 px-6 py-4 shadow-[0_4px_20px_rgba(0,0,0,0.5)] transition-all duration-500">
                            <div className="container mx-auto flex flex-wrap justify-between items-center">

                                {/*  ANIMATED NAVBAR TITLE (Themed Gold) */}
                                <div className="cursor-pointer" onClick={() => setActiveTab('rules')}>
                                    <StrangerTypewriter text="STRANGER TECH" className="text-2xl md:text-3xl !text-[#d4af37]" />
                                </div>

                                <ul className="flex items-center space-x-2 md:space-x-4 text-[10px] font-bold tracking-[0.2em] uppercase font-crimson">
                                    {['rules', 'about', 'help'].map((tab) => (
                                        <li key={tab}>
                                            <button
                                                onClick={() => setActiveTab(tab as any)}
                                                className={`px-4 py-2 rounded-md transition-all ${activeTab === tab
                                                        ? 'text-[#d4af37] bg-[#8b6e2e]/20 border border-[#d4af37]/40 shadow-[0_0_10px_rgba(212,175,55,0.1)]'
                                                        : 'text-[#f2e0b5]/60 hover:text-[#d4af37]'
                                                    }`}
                                            >
                                    ...
                                    {session ? (
                                        <div className="flex items-center gap-4">
                                            <div className="hidden md:block text-right">
                                                <p className="text-[9px] text-[#d4af37] uppercase font-bold tracking-[0.2em] font-crimson opacity-60 leading-none">Identity Verified</p>
                                                <p className="text-xs text-[#f2e0b5] font-script tracking-wide truncate max-w-[150px]">{session.user.email}</p>
                                            </div>
                                            <button
                                                onClick={handleLogout}
                                                className="p-2 border border-[#8b6e2e]/40 hover:border-[#d4af37] text-[#d4af37]/60 hover:text-[#d4af37] hover:bg-[#8b6e2e]/20 rounded-full transition-all"
                                        ...
                                        <button
                                            ...
                                            className="px-5 py-2 bg-gradient-to-tr from-[#3d2618] to-[#8b6e2e] text-[#d4af37] text-[10px] font-bold tracking-[0.2em] rounded-lg border border-[#d4af37]/30 shadow-lg hover:from-[#8b6e2e] hover:to-[#3d2618] transition-all uppercase font-wizard"
                                        >
                                            LOGIN / SIGNUP
                                        </button>
                                    )}
